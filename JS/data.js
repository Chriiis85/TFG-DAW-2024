...
let select3Group = document.getElementById("select3Group");
...
function mostrarResultados(
  ...
  if (opcionSeleccionada2 == "Constructors") {
    ...
    select3Group.style.display = "none";
    ...
    select3Group.style.display = "block";
    obtenerRacesResult(opcionSeleccionada1, opcionSeleccionada3).then(
      (resultados) => {
        //CREAR LA TABLA MEDIANTE JS DINAMICAMENTE Y PINTAR LOS DATOS
        let tablaTBody = document.getElementById("tabla-tbody");
        tablaTBody.innerHTML = "";
        ...
        titulo.textContent =
          opcionSeleccionada1 +
          " " +
          select3.options[select3.selectedIndex].textContent +
          " Results";
      }
    );
  }
...
function obtenerCarreras(opcionSeleccionada) {
  ...
  obtenerRacesYear(opcionSeleccionada).then((resultados) => {
    ...
    //OBTENER EL DIA ACTUAL PARA MOSTRAR SOLO LAS CARRERAS QUE SE HAN ACABADO
    let today = new Date();
    let year = today.getFullYear();
    let month = String(today.getMonth() + 1).padStart(2, "0"); // Los meses son 0-indexados, por lo que sumamos 1
    let day = String(today.getDate()).padStart(2, "0");

    let fechaHoyFormateada = `${year}-${month}-${day}`;

    for (let i = 0; i < resultados.length; i++) {
      if (resultados[i]["date"] < fechaHoyFormateada) {
